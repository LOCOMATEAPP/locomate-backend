import { AdminMallRepository } from './repository';
import { CreateMallInput, UpdateMallInput, CreateFloorInput, UpdateFloorInput } from './schema';

export class AdminMallService {
  private repository: AdminMallRepository;

  constructor() {
    this.repository = new AdminMallRepository();
  }
  ...
  async getMalls(page: number = 1, limit: number = 10) {
  ...
  async createFloor(data: CreateFloorInput) {
  ...
  async updateFloor(id: string, data: UpdateFloorInput) {
